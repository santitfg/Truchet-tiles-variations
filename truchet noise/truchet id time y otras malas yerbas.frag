...
void main(){
    ...
    id+=vec2(u_time*(random2(id)-.5)*.0000000);
    float reductor_rand=.4;
    float id_t=(u_time*(random2(id)-.5)*reductor_rand).x+(u_time*(random2(id)-.5)*reductor_rand).y;
    id_t/=2.;
    st*=div;
    st=fract(st);
    
    st-=.5;
    st=rotacion(st,step(.270,id_t)*.5*PI);
    st+=.5;
    
    float diametro=1.232;
    
    id+=vec2(u_time*(random2(id)-.5)*.4);
    
    st=mix(st,vec2(noise2(id*vec2(.980,.940)+st*vec2(5.552,10.))),.108);
