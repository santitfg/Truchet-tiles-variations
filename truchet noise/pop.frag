...
void main(){
    ...
    st=rotacion(st,random(id_div+u_time*.0000001)*PI);
    st+=.5;
    
    st+=noise2(st*3.692)*.3;
    st+=noise2(st*1.572)*.05;
    st+=noise2(id+u_time*.1)*1.038;
    ...
    color.g*=fract(random(id+.1));
    color.b*=fract(random(id-.1));
